...
// lsm__file_open serves as entry point for LSM based profiling. It captures the
// stack of the task triggering the file_open hook and always allows the
// operation (returns 0). The LSM context is not a struct pt_regs, so the
// user-mode registers are resolved from the current task by passing NULL.
SEC("lsm/file_open")
int lsm__file_open(void *ctx)
{
